...
public class SignUpController {
    ...
    public void showUnMaskedPassword_Field1() {
        if ( checkBox1.isSelected() ) {
            ...
            passwordVisibility_ImageView1.setImage( isNotVisible_PNG );
            unMaskedPassword_Label.setVisible( false );
        }
    }
    ...
    private void verifyPasswordInput() {
        ...
        if ( verifyPassword_MinLength() && verifyNonEmpty_PWField() )
        { hasMinReq_Label.setTextFill( Color.web( green ) ); }
        else { hasMinReq_Label.setTextFill( Color.web( gray ) ); }

        if ( verifyPassword_FieldsMatch() && verifyNonEmpty_PWField() )
        { pwFieldsMustMatch_Label.setTextFill( Color.web( green ) ); }
